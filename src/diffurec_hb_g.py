...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps):
    ...
    if schedule_name == "linear":
        ...
        beta_end = scale * 0.02+ 0.01 # 0.02+ 0.01
        if beta_end > 1:
            beta_end = scale * 0.001 + 0.01 # 0.001 + 0.01
        return np.linspace(beta_start, beta_end, num_diffusion_timesteps, dtype=np.float64)
...
class PositionwiseFeedForward(nn.Module): 
    ...
    def __init__(self, hidden_size, dropout=0.1):
        ...
        self.w_1 = nn.Linear(hidden_size, hidden_size * 4) #4
        self.w_2 = nn.Linear(hidden_size * 4, hidden_size) #4
        self.dropout = nn.Dropout(dropout)
        self.init_weights()
...
class TransformerBlock(nn.Module): # macaron ver
    def __init__(self, hidden_size, attn_heads, dropout):
        ...
        self.feed_forward2 = PositionwiseFeedForward(hidden_size=hidden_size, dropout=dropout)
    ...
    def forward(self, hidden, mask):
        ...
        hidden = self.output_sublayer(hidden, self.feed_forward2)
...
class Diffu_xstart(nn.Module):
    def __init__(self, hidden_size, args):
        ...
        time_embed_dim = self.hidden_size * 4#4
    ...
    def forward(self, rep_item, x_t, t, mask_seq):
        ...
        x_t = x_t + emb_t
        lambda_uncertainty = (self.noise_distribution(rep_item))*self.lambda_uncertainty 
...
class DiffuRec(nn.Module):
    ...
    def q_sample(self, x_start, t, noise=None, mask=None):
        ...
        """
        if noise is None:
            noise = self.noise_distribution(x_start)
    ...
    def p_sample(self, item_rep, noise_x_t, t, mask_seq): 
        ...
        noise1 = th.randn_like(noise_x_t)
        noise = self.noise_distribution(noise_x_t)
    ...
    def reverse_p_sample(self, item_rep, noise_x_t, mask_seq):
        ...
        print('num_Time', self.num_timesteps)
    ...
    def forward(self, item_rep, item_tag, mask_seq):
        ...
        noise = self.noise_distribution(item_tag)
